package com.example.driverschool.utils;
...
public class Cookies {
    ...
    public static void setCookies (HttpServletRequest httpServlet , String user , HttpServletResponse httpResponse) {
        ...
        if ( cookies != null ) {
            for (int i = 0; i < cookies.length; i++) {
                ...
                //  提取名字
                if (name.equals("WeiPeopleUser")) {
                    //  看名字是否相等 ， 相等则不用操作Cookies
                    return;
                }
            }
        }
        Cookie cookie = new Cookie( "WeiPeopleUser" , user ) ;
        //  设置cookie时间
    ...
    public static void  deleteCookies (HttpServletResponse httpServletResponse) {
        Cookie cookie = new Cookie("WeiPeopleUser" , null ) ;
        //  设置cookie时间
        cookie.setMaxAge( 0 ) ;
        //  必须设置路径，否则其他项目路径访问不了
        cookie.setPath("/");
